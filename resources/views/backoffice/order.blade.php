...
<div class="col-12">
  <div class="card card-custom gutter-b">
    <div class="card-header border-0">
      <h3 class="card-title font-weight-bolder text-dark">Order</h3>
    ...
    <div class="card-body">
      <form id="FilterForm" method="get">
        <div class="row">
          ...
          <div class="col-12">
            ...
            <a href="/order/create/"><button type="button" class="btn btn-primary"><i class="fas fa-plus-circle"></i></button></a>
      ...
      <div class="pagination mb-2">
        ...
        <a href="/order/{{ $i }}/page/"><button type="button" class="btn btn-secondary mr-2 {{ ($i == $page?'active':'') }}">{{ $i }}</button></a>
        @endfor
      </div>
      <div id="table">
        <table>
          <tr>
            ...
            <th class="text-center" width="15%">ผู้ซื้อ</th>
            <th class="text-center" width="15%">การจัดส่ง</th>
            <th class="text-center" width="10%">มูลค่ารวม</th>
            <th class="text-center" width="10%">สถานะ</th>
            <th class="text-center" width="15%">วันที่สั่งซื้อ</th>
            <th width="15%"></th>
          </tr>
          ...
          <tr class="{{ ($i % 2 == 0)?'odd':'' }}">
            ...
            <td class="text-center">{{ $order->cust_name }}</td>
            <td class="text-center">{{ $order->ship_name }}</td>
            <td class="text-center">{{ number_format($order->total , 2) }}</td>
            <td class="text-center"><span class="status {{ $order->status }}">{{ $order->status }}</span></td>
            <td class="text-center">{{ $order->created_at->format('d/m/Y H:i A') }}</td>
            <td class="text-right">
              <a href="/order/{{ $order->id }}/edit/"><button class="btn btn-success" title="แก้ไข"><i class="fa fa-edit"></i></button></a>
              <form class="d-inline-block" action="/order/{{ $order->id }}/" method="post">
                @method('DELETE')
                @csrf
