...
<div class="col-12">
  <div class="card card-custom gutter-b">
    <div class="card-header border-0">
      <h3 class="card-title font-weight-bolder text-dark">User</h3>
    ...
    <div class="card-body">
      <form id="FilterForm" method="get">
        <div class="row">
          ...
          <div class="col-12">
            ...
            <a href="/user/create/"><button type="button" class="btn btn-primary"><i class="fas fa-plus-circle"></i></button></a>
      ...
      <div class="pagination mb-2">
        ...
        <a href="/user/{{ $i }}/page/"><button type="button" class="btn btn-secondary mr-2 {{ ($i == $page?'active':'') }}">{{ $i }}</button></a>
        @endfor
      </div>
      <div id="table">
        <table>
          ...
          <tr class="{{ ($i % 2 == 0)?'odd':'' }}">
            ...
            <td class="text-center"><span class="status {{ (empty($user->email_verified_at))?'denied':''}}">{{ (! empty($user->email_verified_at))?'Verified':'Not Verify'}}</span></td>
            <td class="text-center">{{ $user->created_at->format('d/m/Y H:i A') }}</td>
            <td class="text-right">
              <a href="/user/{{ $user->id }}/edit/"><button class="btn btn-success" title="แก้ไข"><i class="fa fa-edit"></i></button></a>
